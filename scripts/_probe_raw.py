...
for label, filename, user_prompt in [
    ...
    pipeline.MODEL_PATH = MODELS_DIR / filename
    pipeline.ensure_server()
    payload = {
        "messages": [
            {"role": "system", "content": pipeline.SYSTEM_PROMPTS["synth"]},
            {"role": "user", "content": user_prompt},
        ],
        "max_tokens": 200,
    }
    resp = requests.post(f"{pipeline.SERVER_URL}/v1/chat/completions", json=payload, timeout=120)
    print(f"=== {label} ===")
